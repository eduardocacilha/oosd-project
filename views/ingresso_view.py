...
class IngressoView:
    ...
    def pega_matricula_comprador(self) -> str:
        try:
            ...
            janela = sg.Window(
                "Matrícula do Comprador", layout, modal=True, finalize=True
            )
            while True:
                evento, valores = janela.read()
    ...
    def confirma_compra_ingresso(self, dados_compra: dict) -> bool:
        try:
            ...
            resposta = sg.popup_yes_no(
                mensagem, title="Confirmar Compra", keep_on_top=True
            )
            return resposta == "Yes"
    ...
    def mostra_ingressos(self, lista_ingressos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Meus Ingressos", layout, resizable=True, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
    ...
    def seleciona_ingresso(self, lista_ingressos: List[dict]) -> Optional[int]:
        try:
            ...
            janela = sg.Window(
                "Selecionar Ingresso", layout, resizable=True, modal=True, finalize=True
            )
            while True:
                evento, valores = janela.read()
                ...
                if evento == "1":
                    try:
                        ...
                        if not indices_selecionados:
                            raise RegraDeNegocioException(
                                "Nenhum ingresso selecionado. Por favor, clique em uma linha da tabela."
                            )
                        else:
    ...
    def pega_novo_preco_revenda(self) -> Optional[float]:
        try:
            ...
            janela = sg.Window(
                "Novo Preço de Revenda", layout, modal=True, finalize=True
            )
            while True:
                evento, valores = janela.read()
                if evento == sg.WINDOW_CLOSED or evento == "-CANCELAR-":
                ...
                if evento == "-OK-":
                    try:
                        ...
                        try:
                            ...
                            raise RegraDeNegocioException(
                                "Preço inválido! Digite um número (ex: 50.00)."
                            )
                    except (
                        RegraDeNegocioException,
                        EntidadeNaoEncontradaException,
