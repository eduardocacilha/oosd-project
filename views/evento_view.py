...
class EventoView:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if evento == "5":
                        ...
                        if not nome:
                            raise RegraDeNegocioException(
                                "Nome do evento não pode estar vazio!"
                            )
                        if not data_str:
                            raise RegraDeNegocioException(
                                "Data do evento deve ser informada!"
                            )
                        try:
                            ...
                            raise RegraDeNegocioException(
                                "Formato de data inválido. Use o calendário ou DD/MM/AAAA."
                            )
                        if not local:
                        ...
                        try:
                            ...
                            raise RegraDeNegocioException(
                                "Preço inválido. Use apenas números (ex: 100.50)."
                            )
                        janela.close()
                        return {
                            "nome": nome,
                    ...
                    self.mostrar_popup(
                        "Erro Inesperado", f"Erro ao processar dados: {e}"
                    )
                    continue
        except Exception as e:
    ...
    def mostra_eventos(self, lista_dados_eventos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Lista de Eventos", layout, resizable=True, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except (EntidadeNaoEncontradaException, RegraDeNegocioException) as e:
            ...
            self.mostrar_popup(
                "Erro Inesperado", f"Erro ao mostrar lista de eventos: {e}"
            )

    def mostra_detalhes_evento(self, dados_detalhados_evento: dict):
        try:
            if not dados_detalhados_evento or not isinstance(
                dados_detalhados_evento, dict
            ):
                raise EntidadeNaoEncontradaException(
                    "Dados detalhados do evento não encontrados."
                )
            try:
            ...
            layout = [
                ...
                [
                    ...
                    sg.Text(
                        f"R$ {float(dados_detalhados_evento.get('preco_entrada', 0)):.2f}"
                    ),
                ],
                [sg.Text("------------------------------------")],
                [
                    sg.Text(
                        "Avaliação Média:", size=(15, 1), font=("Helvetica", 10, "bold")
                    ),
                    sg.Text(nota_str),
                ],
                [sg.Text("------------------------------------")],
    ...
    def mostra_feedbacks(self, lista_dados_feedbacks: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Feedbacks do Evento", layout, resizable=True, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except (EntidadeNaoEncontradaException, RegraDeNegocioException) as e:
    ...
    def seleciona_evento(self, lista_dados_eventos: List[dict]) -> Optional[int]:
        try:
            if not lista_dados_eventos or not isinstance(lista_dados_eventos, list):
                raise EntidadeNaoEncontradaException(
                    "Nenhum evento disponível para selecionar."
                )
            headings = ["#", "Nome", "Data", "Local", "Preço (R$)"]
            dados_tabela = []
            for i, dados in enumerate(lista_dados_eventos, 1):
            ...
            janela = sg.Window(
                "Selecionar Evento", layout, resizable=True, modal=True, finalize=True
            )
            while True:
                try:
                    evento, valores = janela.read()
                    ...
                    if evento == "2":
                        ...
                        if not indices_selecionados:
                            raise RegraDeNegocioException(
                                "Nenhum evento selecionado. Por favor, clique em uma linha da tabela."
                            )
                        else:
            ...
            self.mostrar_popup(
                "Erro Inesperado", f"Erro geral ao selecionar evento: {e}"
            )
            return None
