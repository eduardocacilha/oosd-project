...
class RelatorioView:
    ...
    def tela_opcoes_eventos(self) -> int:
        try:
            ...
            janela = sg.Window(
                "Relatórios de Eventos", layout, finalize=True, modal=True
            )
            while True:
                evento, valores = janela.read()
                if evento == sg.WINDOW_CLOSED:
    ...
    def tela_opcoes_produtos(self) -> int:
        try:
            ...
            janela = sg.Window(
                "Relatórios de Produtos", layout, finalize=True, modal=True
            )
            while True:
                evento, valores = janela.read()
                if evento == sg.WINDOW_CLOSED:
    ...
    def tela_opcoes_vendas(self) -> int:
        try:
            ...
            janela = sg.Window(
                "Relatórios de Vendas", layout, finalize=True, modal=True
            )
            while True:
                evento, valores = janela.read()
                if evento == sg.WINDOW_CLOSED:
    ...
    def tela_opcoes_usuarios(self) -> int:
        try:
            ...
            janela = sg.Window(
                "Relatórios de Usuários", layout, finalize=True, modal=True
            )
            while True:
                evento, valores = janela.read()
                if evento == sg.WINDOW_CLOSED:
    ...
    def mostra_eventos_preco(self, mais_caros: List[dict], mais_baratos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Relatório: Eventos por Preço", layout, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
    ...
    def mostra_eventos_avaliacao(self, eventos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Relatório: Eventos por Avaliação",
                layout,
                resizable=True,
                modal=True,
                finalize=True,
    ...
    def mostra_produtos_preco(self, mais_caros: List[dict], mais_baratos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Relatório: Produtos por Preço", layout, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
    ...
    def mostra_produtos_faturamento(self, produtos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Relatório: Faturamento de Produtos",
                layout,
                resizable=True,
                modal=True,
                finalize=True,
    ...
    def mostra_relatorio_estoque(self, produtos: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Relatório: Estoque de Produtos", layout, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
        ...
        try:
            ...
            for metodo, valor in dados.items():
                try:
                    ...
                    percentual = (
                        valor_float / total_geral * 100 if total_geral > 0 else 0
                    )
                    texto_relatorio += (
                        f"{metodo}: R$ {valor_float:.2f} ({percentual:.1f}%)\n"
                    )
                except (ValueError, TypeError) as e:
            ...
            layout = [
                ...
                [
                    sg.Multiline(
                        texto_relatorio, size=(40, 10), disabled=True, no_scrollbar=True
                    )
                ],
                [sg.Button("Fechar")],
            ]
            janela = sg.Window(
                "Relatório: Vendas por Pagamento", layout, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
    ...
    def mostra_faturamento_evento(self, eventos: List[dict]):
        try:
            ...
            for i, evento in enumerate(eventos, 1):
                try:
                    faturamento_ingressos = float(
                        evento.get("faturamento_ingressos", 0)
                    )
            ...
            janela = sg.Window(
                "Relatório: Faturamento por Evento",
                layout,
                resizable=True,
                modal=True,
                finalize=True,
    ...
    def mostra_top_clientes(self, clientes: List[dict]):
        try:
            ...
            layout = [
                [
                    sg.Text(
                        "======== TOP CLIENTES ========", font=("Helvetica", 14, "bold")
                    )
                ],
                [
                    sg.Table(
    ...
    def mostra_relatorio_geral(self, dados: dict):
        try:
            ...
            try:
                texto_estatisticas += (
                    f"   Usuarios cadastrados: {int(dados.get('total_usuarios', 0))}\n"
                )
                texto_estatisticas += (
                    f"   Eventos cadastrados: {int(dados.get('total_eventos', 0))}\n"
                )
                texto_estatisticas += (
                    f"   Produtos cadastrados: {int(dados.get('total_produtos', 0))}\n"
                )
            ...
            layout = [
                ...
                [
                    sg.Multiline(
                        texto_destaques, size=(50, 5), disabled=True, no_scrollbar=True
                    )
                ],
                [sg.Button("Fechar")],
            ]
            janela = sg.Window(
                "Relatório Geral do Sistema", layout, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
