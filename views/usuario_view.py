...
class UsuarioView:
    ...
    def mostra_usuarios(self, lista_dados_usuarios: List[dict]):
        try:
            ...
            janela = sg.Window(
                "Lista de Usuários", layout, resizable=True, modal=True, finalize=True
            )
            janela.read()
            janela.close()
        except Exception as e:
    ...
    def pega_dados_avaliacao(self) -> dict:
        try:
            layout = [
                ...
                [
                    sg.Text("Nota (1-5):", size=(10, 1)),
                    sg.Input(size=(5, 1), key="-NOTA-"),
                ],
                [sg.Text("Comentário:")],
    ...
    def mostra_usuario(self, dados_usuario: dict):
        try:
            layout = [
                [
                    sg.Text(
                        "--- DETALHES DO USUÁRIO ---", font=("Helvetica", 14, "bold")
                    )
                ],
                [
